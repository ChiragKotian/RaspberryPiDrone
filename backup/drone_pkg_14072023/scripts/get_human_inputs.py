...
def callback(data):
    ...
    pub=rospy.Publisher("/drone1/quad/human_inputs",human_inputs,queue_size=0)
    pub.publish(channels)



def listener():
    rospy.init_node('get_rc1', anonymous=True)
    rospy.Subscriber("/drone1/mavros/rc/in", RCIn, callback)
    rospy.spin()
