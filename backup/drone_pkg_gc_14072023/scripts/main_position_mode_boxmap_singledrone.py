...
def determine_yaw():
    ...
    scale = ((Channels[3]-1500.0)/500.0)*10.0  #5.0 determines max speed in degrees/sec and set 10.0 as per the spped at which your script is running at
...
if __name__ == "__main__":
    ...
    rospy.init_node("offb_node_py1")

    state_sub = rospy.Subscriber("/drone1/mavros/state", State, callback = state_cb)

    rospy.Subscriber("/drone1/quad/quad_states", quad_states, quad_states_callback)
    rospy.Subscriber("/drone1/quad/human_inputs", human_inputs, channels_callback)

    rospy.wait_for_service("/drone1/mavros/cmd/arming")
    arming_client = rospy.ServiceProxy("/drone1/mavros/cmd/arming", CommandBool)

    rospy.wait_for_service("/drone1/mavros/set_mode")
    set_mode_client = rospy.ServiceProxy("/drone1/mavros/set_mode", SetMode)

    local_pos_pub = rospy.Publisher("/drone1/mavros/setpoint_position/local", PoseStamped, queue_size=10)
    ...
    while(not rospy.is_shutdown() ):
        ...
        coordinates[0]= ((Channels[1] - 1500.0)/500.0)* boxlength*-1
        coordinates[1]= ((Channels[0] - 1500.0)/500.0)* boxlength*-1
        ...
        print(" current_x -> " + str(quad_state[0]) + " current_y -> " + str(quad_state[1]) + " current_z -> " + str(quad_state[2]) + " current_yaw -> "+ str(quad_state[5]))
