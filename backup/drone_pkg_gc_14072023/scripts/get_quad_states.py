...
quad_position_global = [0.0, 0.0, 0.0] #latitude,longitude,altitude
...
def callback_GPS_local(data):
    global quad_position_local
    global quad_state_assign
    global quad_velocity_local
...
def callback_GPS_global(data):
    global quad_position_global
    global quad_state_assign

    quad_position_global = [data.latitude,data.longitude,data.altitude]
    quad_state_assign.quad_latitude = quad_position_global[0]
    quad_state_assign.quad_longitude = quad_position_global[1]
    quad_state_assign.quad_altitude = quad_position_global[2]
...
def callback(data):
    ...
    pub=rospy.Publisher("/drone1/quad/quad_states",quad_states,queue_size=0)
...
def listener():
    ...
    rospy.init_node('get_imu1', anonymous=True)
    rospy.Subscriber("/drone1/mavros/imu/data", Imu, callback)
    rospy.Subscriber("/drone1/mavros/global_position/local", Odometry, callback_GPS_local)
    rospy.Subscriber("/drone1/mavros/global_position/global", NavSatFix, callback_GPS_global)
